---
name: docwright-estimate
description: |
  CRITICAL: Use for estimating work effort from docwright Task Contracts. Triggers on:
...
> **Version:** 1.2.0 | **Last Updated:** 2026-06-08 | **Tracks docwright:** 0.3.0 (BDD-spine)
...
|--------|---------|--------|
| Estimate a spec | `docwright contract <spec>` then apply estimation | Round-based breakdown table |
| Batch estimate | Run on all contracts in `docs/` | Sorted effort ranking |
| Calibrate from history | `docwright explain <spec> --history` | Compare predicted vs actual rounds |
| Library sizing signal (0.3.0) | `docwright audit --spec-dir docs --format json` | Counts of rules/scenarios/unproven-rules/open-questions to weight remaining effort |
...
```bash
docwright contract docs/features/<goal>/spec.md
```
...
```bash
for spec in docs/features/*/spec.md; do
  echo "=== $(basename $spec) ==="
  docwright contract "$spec" 2>/dev/null | head -20
  echo
done
```
...
Use `docwright graph` to visualize spec dependencies and critical path before estimating a batch:
