...
场景: 路径都不在仓库内时不再摸到进程 cwd 的仓库
  测试:
    包: agent-spec
    过滤: test_repo_root_discovery_ignores_process_cwd
  假设 spec 目录与 code 目录都是临时的非 git 目录且进程 cwd 位于一个 git 仓库内
  当 解析 guard 的仓库根
  那么 返回 None

场景: 非 git 目录返回空变更集且不受宿主暂存状态影响
  测试:
    包: agent-spec
    过滤: test_resolve_guard_change_paths_returns_empty_outside_git_repo
  假设 spec 目录与 code 目录都是临时的非 git 目录且宿主仓库存在暂存变更
  当 以 staged scope 解析 guard 变更集
  那么 返回空变更集
